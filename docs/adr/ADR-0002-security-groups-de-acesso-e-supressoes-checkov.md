...
| `CKV_AWS_24`    | `aws_vpc_security_group_ingress_rule.lab_ssh` | Falso positivo por forma de recurso não modelada: o check lê apenas `security_groups` e `source_security_group_id` — campos dos recursos legados — e desconhece `referenced_security_group_id`; sem origem nos campos que conhece, conclui "aberto ao mundo". Medido no fonte (`AbsSecurityGroupUnrestrictedIngress.py`, 107-110) e em teste de três variantes na porta 22: a referência SG↔SG falha e `cidr_ipv4 = "10.0.0.0/24"` passa — o check aprova a forma mais permissiva. A origem em SG é exigência de §5.2; troca por `cidr_ipv4` está vedada |

**Errata de 2026-08-14, pós-aprovação.** `CKV_AWS_24` foi acrescentado após a implementação, carimbado sob a delegação registrada no campo *Decisor*. Omissão em lista enumerada, mesma classe dos itens já presentes — nenhuma decisão de §5.2 ou §5.3 mudou. As duas predições de §16 se resolveram: `CKV2_AWS_5` disparou como previsto; `CKV_AWS_23` passou (7 passed, 0 failed).
